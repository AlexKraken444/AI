...
test("new mode choices persist after the one-time upgrade", () => {
  for (const modelMode of Object.keys(models.names)) assert.equal(models.restore({modelMode, modelRevision: 4}), modelMode);
  assert.equal(models.restore({modelMode: "reference"}), "reference");
});
test("fresh or invalid settings use the new model", () => {
  assert.equal(models.restore(null), "context4");
  assert.equal(models.restore({modelMode: "unknown", modelRevision: 4}), "context4");
  assert.equal(models.restore({modelMode: "toString", modelRevision: 4}), "context4");
});
